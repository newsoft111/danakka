...
async def get_user_ticket_history(
	...
	# 유저의 티켓 사용 내역 조회
	ticket_usage_history = db.query(TicketModels.TicketUsageHistory).filter_by(auth_user_id=authorized_user.id).all()
	
	# 유저의 티켓 충전 내역 조회
	ticket_purchase_history = db.query(TicketModels.TicketPurchaseHistory).filter_by(auth_user_id=authorized_user.id).all()
